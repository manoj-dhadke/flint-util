 ...
 * Summary: HEAD method of HTTP
 * Description: To HEAD data from a URL using HTTP Connector
 */

//Starting execution of flintbit
log.trace("Started executing 'flint-util:http:operation:workflow:head.js'");
...
if(response_exitcode==0){
    ...
    log.info("Method result:"+response_headers);
    //User Message
    user_message = "Response-Headers: "+response_headers;
    output.set("exit-code",0).set("result",response_headers).set("user_message",response_headers);
    log.trace("Finished executing 'flint-util:http:operation:workflow:head.js' successfully")
}

else{
    log.error("Failure in execution of method:"+method);
    output.set("error",response_message).set("exit-code",response_exitcode);
    log.trace("Finished executing 'flint-util:http:operation:workflow:head.js' with errors");
}