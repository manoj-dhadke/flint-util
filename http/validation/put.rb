...
# begin
@log.trace("Started executing 'flint-util:http:validation:put.rb' flintbit...")
begin
    ...
    @request_method = 'put' if @request_method.nil? || @request_method.empty?
    if @request_url.nil? || @request_url.empty?
        @request_url = 'http://httpbin.org/put'
    end
    ...
    if @request_timeout.to_s.empty?
      ...
      response = all_connector.set('timeout', request_timeout).sync
    end
...
@log.trace("Finished executing 'flint-util:http:validation:put.rb' flintbit...")
# end
