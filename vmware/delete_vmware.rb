#begin
@log.trace("Started execution 'fb-cloud:vmware55:operation:delete_virtual_machines.rb' flintbit...") # execution Started
begin
    # Flintbit input parametes
    #Mandatory
    @connector_name = @config.global('vmware.connector_name') # vmware connector name
    @action = 'delete-vm' # name of the operation: delete-vm
    ...
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "VMWare connector name (connector_name)" to delete virtual machines'
    end

    #checking virtual machine name is nil or empty
    if @vmname.nil? || @vmname.empty?
        raise 'Please provide "Virtual Machine name (@vmname)" to delete virtual machines'
    else
