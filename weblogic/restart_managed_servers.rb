@log.trace("Started executing flint-mmc:weblogic:restart_managed_servers.rb' flintbit...")
begin
    ...
    @log.info("----Result start managed servers---::: #{result}")
    if response.exitcode == 0
        @log.info("SUCCESS in executing #{@connector_name} where, exitcode :: #{response_exitcode} | 
                                                    message ::  #{response_message}")
        @output.set("exit-code",response_exitcode).set("message",response_message)
        @log.info("Successfully started managed servers")
        user_message = """## Successfully started managed servers"""
        @output.set('user_message', user_message)
    else
        @log.error("ERROR in executing #{@connector_name} where, exitcode :: #{response_exitcode} | 
                                                            message ::  #{response_message}")
        @output.set('exit-code', 1).set('message', response_message)
        @log.info("Failed to start managed servers")
        user_message = """## Failed to start managed servers"""
        @output.set('user_message', user_message)
    end
    ...
    @log.info("There is an issue while starting managed servers")
    user_message = """## There is an issue while starting managed servers"""
    @output.set('user_message', user_message)
end
@log.trace("Finished executing 'flint-mmc:weblogic:restart_managed_servers.rb' flintbit...")