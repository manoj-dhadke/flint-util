...
require 'json'
# begin
@log.trace("Started executing 'zendesk:list_comments.rb' flintbit...")
begin
    ...
    @action = 'list-comments' # Contains the name of the operation: list-comments
    ...
    @log.info("Flintbit input parameters are, connector name :: #{@connector_name} |action :: #{@action} | ticket-id :: #{@id}")
 ...
 if @request_timeout.nil? || @request_timeout.is_a?(String)
    ...
    if response_exitcode == 0
        ...
        comment_json = response_body.get('comments')
        comment_json.each do |iterator|
        @log.info("Result : #{iterator}")
        comment_id = iterator.get('id')
...
@log.trace("Finished executing 'zendesk:list_comments.rb' flintbit...")
# end
