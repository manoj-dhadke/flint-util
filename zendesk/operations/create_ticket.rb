...
# begin
@log.trace("Started executing 'zendesk:create_ticket.rb' flintbit...")
begin
    ...
    @action = 'create-ticket' # Contains the name of the operation: create-tickets
    @data ={}     # Hash to store Key, Value pairs
    @custom_fields = Array.new    # The Array of custom fields
    @custom_field = {} # Hash to store Key, Value pairs
    @comment = {} # Hash to store Key, Value pairs
    @data['description'] = @input.get('description')
    @data['priority'] = @input.get('priority')
    @data['status'] = @input.get('status')
    @data['recipient'] = @input.get('recipient')
    @data['assignee_id'] = @input.get('assignee_id')
    @data['type'] = @input.get('type')
    @data['subject'] = @input.get('subject')
    @data['requester_id'] = @input.get('requester_id')
    @data['problem_id'] = @input.get('problem_id')
    @data['collaborator_ids'] = @input.get('collaborator_ids')
    @comment['body'] = @input.get('comment')
    @data['comment'] = @comment
    @custom_field['id'] =  @input.get('custom_fields_id')
    @custom_field['value'] = @input.get('custom_fields_value')
...
@log.trace("Finished executing 'zendesk:create_ticket.rb' flintbit...")
# end
