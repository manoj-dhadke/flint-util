...
# begin
@log.trace("Started executing 'zendesk:make_comment_private.rb' flintbit...")
begin
    ...
    @action = 'make-comment-private' # Contains the name of the operation: make-comment-private
    @id = @input.get('ticket-id')   #Provide Ticket Id
    @comment_id = @input.get('comment-id') #Provide Comment Id
    ...
    @log.info("Flintbit input parameters are, connector name :: #{@connector_name} |action :: #{@action} | ticket-id :: #{@id} | comment-id :: #{@comment_id}")
 ...
 if @request_timeout.nil? || @request_timeout.is_a?(String)
    ...
    if response_exitcode == 0
        ...
        @output.set('result', response_message).set('exit-code', response_exitcode)
    else
...
@log.trace("Finished executing 'zendesk:make_comment_private.rb' flintbit...")
# end
