...
# begin
@log.trace("Started executing 'flint-util:winrm:ntlm:date.rb' flintbit...")
begin
    hostname = @input.get('hostname')
    protocol = @input.get('protocol')
    username = @input.get('username')
    password = @input.get('password')
    bit_response = @call.bit('flint-util:winrm:connect:ntlm_connect.rb').set('command', 'date /t && time /t').set('hostname', hostname)
...
@log.trace("Finished execution of 'flint-util:winrm:ntlm:date.rb' flintbit...")
# end
