...
# begin
@log.trace("Started executing 'flint-util:winrm:basic:ipconfig.rb' flintbit...")
begin
    hostname = @input.get('hostname')
    protocol = @input.get('protocol')
    username = @input.get('username')
    password = @input.get('password')
    bit_response = @call.bit('flint-util:winrm:connect:basic_auth_connect.rb').set('command', 'ipconfig').set('hostname', hostname)
...
@log.trace("Finished execution of 'flint-util:winrm:basic:ipconfig.rb' flintbit...")
# end
