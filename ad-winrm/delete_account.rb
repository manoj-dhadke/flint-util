@log.trace("Started executing 'flint-util:ad-winrm:delete_account.rb' flintbit...")
begin
	...
	if flintbit_response.get("exit-code") == 0
		...
		@user_message = """**Account with username :'#{@login_name}' is deleted successfully**"""
		@log.info("Account deletion operation successful    ::user_message:: #{@user_message}")
		...
		@log.error("Account deletion operation unsuccessful")
...
#{@a.rstrip}"""
	...
	@log.error(e.message)
	@user_message = """**Failed to delete user account !** """
	@output.set('message', e.message).set('exit-code', -1).set('user_message',@user_message)
	@log.info('output in exception')
	@output.exit(1,e.message)
end
@log.trace("Finished executing 'flint-util:ad-winrm:delete_account.rb' flintbit")
