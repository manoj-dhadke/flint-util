...
require 'json'
begin
	@log.trace("Started execution of 'flint-util:jenkins:jenkins_start_build.rb'flintbit..")
	...
	if @build_exitcode == 0
		 @user_message= "The build request for build name: #{@build_name} is completed successfully"
		#@reply_message = 'Hello @' + @mention_name + ',The build request for ' + @build_name + ' is complete.'
		@output.set('exit-code',@build_exitcode).set('user_message',@user_message)
	else
		@user_message= "The build request for build name: #{@build_name} failed due to : #{@build_message}"
...
@log.trace("Finished execution of 'flint-util:jenkins:jenkins_start_build.rb'flintbit..")
