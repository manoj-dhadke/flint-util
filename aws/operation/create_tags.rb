#begin
@log.trace("Started executing 'create_tags' flintbit...")
begin
...
if connector_name.nil? || connector_name.empty?
   raise 'Please provide Amazon EC2 "connector name (connector_name)" to create Tags'
end

if resource_id.nil? || resource_id.empty?
   raise 'Please provide "Amazon EC2 resource ID (resource_id)" to create Tags'
end

if tag_key.nil? || tag_key.empty?
   raise 'Please provide Amazon EC2 "Tag Key (tag_key)" to create Tags'
end

if tag_value.nil? || tag_value.empty?
	raise 'Please provide "Tag Value (tag_value)" to create Tags'
end
...
if response_exitcode == 0
	...
	@log.trace("Finished executing 'create_tags' flintbit with success...")

else
	@log.error("Failure in executing Amazon EC2 Connector where, exitcode :: #{response_exitcode} |
																															 message ::  #{response_message}")
  @output.set("error",response_message)
  @log.trace("Finished executing 'create_tags' flintbit with error...")	
end
rescue Exception => e
  @log.error(e.message)
end
