#begin
@log.trace("Started executing 'describe_amazon_instance' flintbit...")
begin
...
if response_exitcode == 0
  ...
  @log.info("Amazon EC2 described instance image id ::             #{instance.get("image-id")} |
	...
	@log.trace("Finished executing 'describe_amazon_instance' flintbit with success...")
else
	@log.error("Failure in executing Amazon EC2 Connector where, exitcode :: #{response_exitcode} |
																															 message ::  #{response_message}")
  @output.set("error",response_message)
  @log.trace("Finished executing 'describe_amazon_instance' flintbit with error...")
end
rescue Exception => e
  @log.error(e.message)
end
