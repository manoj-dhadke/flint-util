...
# begin
@log.trace("Started executing 'flint-util:cloud:operation:describe_instance.rb' flintbit...")
begin
      ...
      result = response.get('describe-instance') # Response Body
      ...
      if response.exitcode == 0
          result.each do |instance_id|
              @log.info("#{@connector_name} Instance id :     #{instance_id.get('id')} |
                                            ...
                                            hostname :        #{instance_id.get('host-name')}|
